...
func main() {
	...
	case "-c":
		...
		GenesisBlock, err := chain.mechanisms.loadGenesisFromFile(createCommand)
		if err != nil {
		...
		ChainID, err := chain.init.generateChainID()
		if err != nil {
		...
		publicKey, privateKey, err := modules.crypto.generateKeyPair()
		if err != nil {
		...
		err = chain.mechanisms.exportEncryptedChain(TestChain, privateKey)
		if err != nil {
	...
	case "-a":
		...
		TargetChain, err := chain.mechanisms.loadEncryptedChain(accessCommand, key)
		if err != nil {
	...
	case "-A":
		...
		TargetChain, err := chain.mechanisms.loadEncryptedChain(blockChainID, key)
		if err != nil {
			fmt.Println("Error loading encrypted chain:", err)
			return
		}

		newBlockData, err := chain.mechanisms.loadBlockFromFile(addBlockCommand)
		if err != nil {
			fmt.Println("Error loading block data:", err)
			return
		}

		addedBlock := chain.init.generateBlock(TargetChain.Head, []interface{}{
			newBlockData.Index,
		...
		chain.mechanisms.addBlockToChain(addedBlock, &TargetChain)
